    ...
    <div className="mt-16 max-w-4xl mx-auto">
      ...
      <div className="border-b border-gray-200">
        <nav className="flex space-x-8">
          <button
            className={`cursor-pointer py-4 px-1 border-b-2 text-sm font-medium ${activeTab === "description" ? "border-pink-500 text-pink-600" : "border-transparent text-gray-500 hover:text-pink-700"}`}
            onClick={() => setActiveTab("description")}
          >
            Description
          </button>
          <button className={`cursor-pointer py-4 px-1 border-b-2 text-sm font-medium ${activeTab === "reviews" ? "border-pink-500 text-pink-600" : "border-transparent text-gray-500 hover:text-pink-700"}`} onClick={() => setActiveTab("reviews")}>
      ...
      {/* Tab Content */}
      <div className="py-8">{activeTab === "description" ? <DescriptionTab product={product} /> : <ReviewSection product={product} reviews={reviews} setReviews={setReviews} avgRating={avgRating} reviewCount={product.reviewCount} />}</div>
    </div>
  );
}
...
function ReviewSection({ product, reviews, setReviews, avgRating, reviewCount }) {
  // Fetch reviews function (memoized)
            ...
            <div className="flex justify-center mt-2">
              {[...Array(5)].map((_, i) => {
                ...
                if (avgRating >= ratingValue) {
                  return <FaStar key={i} className="w-5 h-5 text-pink-400 fill-current" />;
                } else if (avgRating >= ratingValue - 0.5) {
                  return <FaStarHalfAlt key={i} className="w-5 h-5 text-pink-400 fill-current" />;
                } else {
